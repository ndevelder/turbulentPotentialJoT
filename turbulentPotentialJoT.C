...
defineTypeNameAndDebug(turbulentPotentialJoT, 0);
addToRunTimeSelectionTable(RASModel, turbulentPotentialJoT, dictionary);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //


tmp<volScalarField> turbulentPotentialJoT::Ts() const
{
	if(tslimiter_ == "true")
	{
...
tmp<volScalarField> turbulentPotentialJoT::TsEh() const
{
	if(tslimiter_ == "true")
	{
...
turbulentPotentialJoT::turbulentPotentialJoT
(
...
tmp<volSymmTensorField> turbulentPotentialJoT::R() const
{
...
// Not used but necessary for RAS Model
tmp<volSymmTensorField> turbulentPotentialJoT::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> turbulentPotentialJoT::divDevReff(volVectorField& U) const
{
    return
    (
       fvc::grad(tpphi_)
...
bool turbulentPotentialJoT::read()
{
    if (RASModel::read())
    {
...
void turbulentPotentialJoT::correct()
{
    ...
    // Display log output if production is negative
    if(timeScaleEps_ == "epsilon" || timeScaleEps_ != "epsHat")
    {
        T = Ts();
	...
	// Production Terms
	const volSymmTensorField PS("PS",tpphi_*(symm(uGrad_)));
	...
	{
		Info<< "Using psi*vort for production" << endl;
